...
DROP_TAGS = {"script", "style", "noscript", "svg", "canvas", "template", "form", "button", "input"}
...
def normalize_url(url: str, *, keep_query: bool = True) -> str:
    ...
    # Fragments identify locations inside the same HTML document, not separate fetches.
...
def matches_rule(url: str, rule: tuple[str, str, str]) -> bool:
    ...
    p = urlsplit(url)
    path = p.path or "/"
    # Include the wildcard root itself and descendants.
...
def safe_output_path(url: str) -> str:
    ...
    host = re.sub(r"[^a-z0-9.-]+", "-", p.netloc.lower()).strip("-") or "unknown-host"
...
class Fetcher:
    def __init__(self, config: dict):
        ...
        self.session.headers.update({
            "User-Agent": "Deck-of-Cardinals-Slate-KB/1.0 (+https://github.com/Deck-of-Cardinals/Slate-KB)",
...
def canonicalize_html(raw_html: str, source_url: str) -> tuple[str, str, list[str], int]:
    # html5lib intentionally tolerates broken/malformed source HTML. We then rebuild a very small,
    # valid subset and discard all source attributes except link targets.
    soup = BeautifulSoup(raw_html, "html5lib")
    title = ""
    ...
    # Reparse only the root and strip risky/invalid attributes/tags.
    ...
    # Avoid unnecessary nested html/body created by html5lib.
    content_html = "\n".join(str(child) for child in body.contents if not isinstance(child, NavigableString) or child.strip())
...
def pdf_to_html(data: bytes, source_url: str) -> tuple[str, str, int]:
    ...
    for i, page in enumerate(reader.pages, start=1):
        ...
        page_html = "".join(f"<p>{html.escape(p).replace(chr(10), '<br>')}</p>" for p in paras)
        blocks.append(f"<section><h2>Page {i}</h2>{page_html}</section>")
    content = "\n".join(blocks)
    title = Path(urlsplit(source_url).path).name or "PDF document"
    return title, content, len(" ".join(BeautifulSoup(content, "html.parser").stripped_strings))
...
def fetch_youtube_transcript(url: str, session: requests.Session) -> tuple[str, str, int] | None:
    ...
    if not video_id:
        return None
    try:
        # oEmbed is public and requires no API key.
        oembed = session.get(
    ...
    try:
        ...
        lines = [getattr(snippet, "text", "").strip() for snippet in transcript if getattr(snippet, "text", "").strip()]
        if not lines:
            return None
        body = "<section><h2>Transcript</h2>" + "".join(f"<p>{html.escape(line)}</p>" for line in lines) + "</section>"
...
a{{color:#0645ad}} table{{border-collapse:collapse;width:100%}} th,td{{border:1px solid #bbb;padding:.4rem;vertical-align:top}}
.source-meta{{font-size:.82rem;color:#555;border-bottom:1px solid #ddd;padding-bottom:1rem;margin-bottom:1.5rem}}
</style>
</head>
<body>
<div class="source-meta"><strong>Source:</strong> <a href="{html.escape(source_url, quote=True)}">{html.escape(source_url)}</a><br><strong>Mirror refreshed:</strong> {html.escape(fetched_at)}</div>
{body_html}
</body>
</html>
"""
...
def rewrite_links(document: str, current_output: str, output_map: dict[str, str], keep_source_links: bool) -> str:
    ...
    for a in soup.find_all("a", href=True):
        ...
        if target:
            rel = os.path.relpath(target, start=current_dir).replace(os.sep, "/")
            a["href"] = rel
...
def make_index(results: list[Result], generated_at: str, title: str) -> str:
    rows = []
    for r in sorted(results, key=lambda x: x.url.lower()):
        link = f'<a href="{html.escape(r.output_path or "", quote=True)}">mirror</a>' if r.output_path else "—"
        rows.append(
            "<tr>"
...
<style>body{{font-family:Arial,Helvetica,sans-serif;max-width:1200px;margin:2rem auto;padding:0 1rem;line-height:1.45}}table{{border-collapse:collapse;width:100%;font-size:.9rem}}th,td{{border:1px solid #bbb;padding:.4rem;vertical-align:top}}th{{text-align:left}}code{{background:#eee;padding:.1rem .25rem}}</style></head><body>
<h1>{html.escape(title)}</h1><p>This site is an automatically generated, simplified HTML mirror for Slate AI ingestion. Last run: <strong>{html.escape(generated_at)}</strong>.</p>
<p>The original source URL is shown on every mirrored page. Failed refreshes preserve the last successful mirrored copy when one already exists.</p>
<table><thead><tr><th>Status</th><th>Source URL</th><th>Mirror</th><th>Note</th></tr></thead><tbody>{''.join(rows)}</tbody></table></body></html>"""
...
def main() -> int:
    ...
    prior_by_url = {item.get("url"): item for item in prior_manifest.get("results", []) if item.get("url")}
    ...
    results: list[Result] = []
    docs_by_url: dict[str, tuple[str, str]] = {}  # normalized URL -> (output path, document)
    ...
    try:
        while queue and len(fetched_urls) < max_total:
            ...
            # Playlist pages and Power BI reports are intentionally recorded rather than pretending their UI chrome is knowledge content.
            p = urlsplit(requested_url)
            if "youtube.com" in p.netloc.lower() and p.path.startswith("/playlist"):
                results.append(Result(url=key, output_path=None, status="unsupported", note="YouTube playlist expansion is not enabled yet."))
                continue
            if p.netloc.lower() == "app.powerbi.com":
                results.append(Result(url=key, output_path=None, status="unsupported", note="Interactive Power BI report; no reliable static text extraction yet."))
                continue
            ...
            try:
                yt = None
                if "youtube.com" in p.netloc.lower() or p.netloc.lower() == "youtu.be":
                ...
                if yt:
                    ...
                    results.append(Result(url=key, output_path=output_path, status="ok", title=title, fetched_at=fetched_at, fetch_method="youtube-transcript", text_chars=text_chars))
                    continue

                final_url, payload, kind, http_status, method = fetcher.get(requested_url)
                final_key = normalize_url(final_url)
                if kind == "pdf":
                    title, body_html, text_chars = pdf_to_html(payload if isinstance(payload, bytes) else payload.encode(), final_url)
                    document = build_document(title, final_key, fetched_at, body_html)
                    docs_by_url[final_key] = (safe_output_path(final_key), document)
                    results.append(Result(url=final_key, output_path=safe_output_path(final_key), status="ok", title=title, fetched_at=fetched_at, fetch_method=method + "+pdf", http_status=http_status, text_chars=text_chars))
                else:
                    ...
                    results.append(Result(url=final_key, output_path=out, status="ok", title=title, fetched_at=fetched_at, fetch_method=method, http_status=http_status, text_chars=text_chars))

                    # If this page belongs to one or more wildcard scopes, follow qualifying descendant links.
                    applicable_rules = [i for i, rule in enumerate(rules) if matches_rule(final_key, rule)]
                    ...
                    for href in discovered:
                        ...
                        if not discovered_key or discovered_key in queued or discovered_key in fetched_urls:
                            continue
                        if not is_discoverable_html_link(discovered_key):
                            continue
                        matching = [i for i in applicable_rules if matches_rule(discovered_key, rules[i]) and per_rule_count[i] < max_per_rule]
                        if not matching:
                            continue
                        chosen = matching[0]
                ...
                if prior_path and (output_dir / prior_path).exists():
                    results.append(Result(url=key, output_path=prior_path, status="stale", fetched_at=fetched_at, note=f"Refresh failed; retained previous copy. {type(exc).__name__}: {exc}"))
                else:
                    results.append(Result(url=key, output_path=None, status="failed", fetched_at=fetched_at, note=f"{type(exc).__name__}: {exc}"))
            finally:
                time.sleep(delay)
    finally:
        fetcher.close()

    # Map all successful pages before rewriting links so mirrored pages point to one another.
    output_map = {u: out for u, (out, _doc) in docs_by_url.items()}
    # Include retained pages from the prior manifest in the rewrite map.
    for item in prior_manifest.get("results", []):
        if item.get("output_path"):
            output_map.setdefault(normalize_url(item.get("url", "")), item["output_path"])

    for url, (out, doc) in docs_by_url.items():
        final_doc = rewrite_links(doc, out, output_map, bool(config.get("keep_source_links", True)))
    ...
    write_text(output_dir / "_status" / "manifest.json", json.dumps(manifest, indent=2, ensure_ascii=False) + "\n")
    write_text(output_dir / "_status" / "heartbeat.txt", f"Last automated refresh: {generated_at}\n")
    write_text(output_dir / "index.html", make_index(results, generated_at, config.get("site_title", "Slate Knowledge Mirror")))
    write_text(output_dir / ".nojekyll", "")
    write_text(output_dir / "robots.txt", "User-agent: *\nAllow: /\n")

    # Simple sitemap for public visibility/debugging.
    base = config.get("site_base_url", "").rstrip("/") + "/"
    urls = [base]
    for r in results:
        if r.output_path:
            urls.append(base + r.output_path)
    ...
    print(f"Mirror run complete: {ok} ok, {stale} stale, {failed} failed, {unsupported} unsupported; {len(results)} total results")
    # A few failures should not prevent publishing other good pages. Fail only if nothing useful was produced.
